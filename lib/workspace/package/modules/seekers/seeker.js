...
module.exports = class extends DynamicProcessor {
	...
	// The bundle object when it is found in the pkg project or a library
	#bundle;
	get bundle() {
		return this.#bundle;
	}
	...
	_process() {
		...
		if (pkg === this.#pkg.package || libraries.has(pkg)) {
			if (subpath === 'config') {
				const vspecifier = (() => {
					if (pkg === this.#pkg.package) return `${this.#pkg.vspecifier}/config`;
		...
		const external = new (require('./external'))(pkg, version, subpath, this.#pkg);
