...
module.exports = class extends DynamicProcessor {
	...
	// Actually only `${package}/config' is a reserved bundle
	#reserved;
	get reserved() {
	...
	// The bundle object when it is found in the package project or a library
	#bundle;
	get bundle() {
		return this.#bundle;
	}
	 ...
	 */
	constructor(pkg, specifier, distribution) {
		if (!pkg || !specifier || !distribution) throw new Error('Invalid parameters');
		...
		super.setup(
			new Map([
				...
				['bundles', { child: pkg.modules.bundles }]
			])
		);
	}
	...
	_process() {
		...
		if (pkg === this.#package.name || libraries.has(pkg)) {
			if (subpath === 'config') {
				const vspecifier = (() => {
					if (pkg === this.#package.name) return `${this.#package.vspecifier}/config`;

					const library = libraries.get(pkg);
					return `${library.vspecifier}/config`;
				})();

				this.#reserved = { name: 'config', vspecifier };
				return;
			}
		...
		const external = new (require('./external'))(pkg, version, subpath, this.#package);
