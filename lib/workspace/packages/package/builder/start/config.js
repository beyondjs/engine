...
module.exports = async function (builder, distribution, path, uglifier, exported) {
	...
	const config = await pkg.config.get(distribution);
	await config.ready;
	const target = require('path').join(path, 'config.js');

	if (distribution.minify.js) {
		const { code, errors } = uglifier.uglify('config.js', config.code);
		errors ? builder.emit('error', 'Error uglifying package styles') : await fs.save(target, code);
		return;
	}
