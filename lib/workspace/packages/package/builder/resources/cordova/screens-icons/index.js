...
module.exports = async function (builder, distribution, path) {
	...
	const sizes = require('./sizes')[distribution.platform];

	let dirname = require('path').join(path, 'resources');
	...
	for (const size of sizes.icons) {
		const target = require('path').join(dirname, 'icons', `${size}.png`);

		// Do not rebuild icon if it was already created
		if (!distribution.icons && (await fs.exists(target))) continue;

		const resource = await pkg.resources.icons.get(parseInt(size), parseInt(size), distribution.icons);
		if (!resource) return;
	...
	for (const size of sizes.screens) {
		...
		const target = require('path').join(dirname, 'screens', `${width}x${height}.png`);

		// Do not rebuild screen if it was already created
		if (!distribution.icons && (await fs.exists(target))) continue;

		const resource = await pkg.resources.screens.get(parseInt(width), parseInt(height), distribution.icons);
