...
module.exports = (packages, specs) => async function (rq, res) {
    ...
    if (!specifier.version) {
        response({content: 'Package version must be set', contentType: 'text/plain', statusCode: 404});
        return;
    }
    const vspecifier = `${specifier.pkg}@${specifier.version}` + (specifier.subpath ? `/${specifier.subpath}` : '');
    console.log('Parsed specifier:', vspecifier);
