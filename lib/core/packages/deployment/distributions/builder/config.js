...
module.exports = async function (builder, distribution, path, modules) {
    ...
    if (!distribution.npm && platforms.node.includes(distribution.platform)) return;
    ...
    let items = {exportName: 'config', extensions: []};

    const promises = [];
    for (const platform of Object.keys(compilePlatforms)) {
        if (platforms.webAndMobile.includes(platform)) {
