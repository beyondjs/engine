...
module.exports = async function (builder, module, {npm, platform, bundles}, paths) {
    ...
    const generate = async items => {
        ...
        for (const item of items) {
            let identifier = typeof item === 'object' ? item.id : item;

            // Hack - compatibility with UImport, expect to receive the bundle with the extension in case you are asked for static
            if (!identifier.endsWith('.js') && !identifier.endsWith('.css')) {
            ...
            if (css) {
                const {code} = await require('uimport')(`${item}.css`, bundles.mode, uiSpecs);
