...
module.exports = async function (builder, distribution, path) {
	...
	for (const al of libraries.values()) {
		...
		const { code, error } = minify(config.code, { mangle: mangle ?? false, compress: compress ?? false });
		if (error) {
			this.emit('error', `Error building config: ${error}`);
			continue;
		}

		promises.push(fs.save(target, code));
	}
