...
module.exports = async function (builder, distribution, path, uglifier) {
	...
	for (const al of libraries.values()) {
		...
		let code, error;
		const specs = Object.assign({ sourceMap: true }, { mangle: mangle ?? false, compress: compress ?? false });
		minify(config.code, specs)
			.then(result => {
				code = result.code;
				error = result.error;
				error ? this.emit('error', `Error building config: ${error}`) : promises.push(fs.save(target, code));
			})
			.catch(error => console.error(error));

		// ({ code, errors } = await uglifier.uglify('start.js', startCode, distribution));
	}
