...
module.exports = async function (mode, packager, processorName, path, builder, language) {
	...
	const bspecifier = `${packager.bundle.container.specifier}${language ? `.${language}` : ''}`;
	const processor = packager.processors.get(processorName);
	if (!processor) {
		// builder.emit('message', `  . Sourcemap not generated: module "${bspecifier}" not have processor "${processorName}"`);
		return;
	}
	...
	try {
		...
		console.error(`Error processing the source maps of the ${bspecifier} module`);
	}
};
