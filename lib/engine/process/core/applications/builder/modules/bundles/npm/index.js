 ...
 * @specs exports {Map} The resources being exported
...
module.exports = async function (specs) {
	...
	const save = async (target, code, packager, extension) => {
		// adding sourcemaps
		code = await require('../sourcemaps')(bundle, code, '.js', path, distribution.maps, extension, language);

		builder.emit('message', `  . File ${bName}.${extension} saved.`);
		await fs.save(target, code);

		// Get first packager to generate bundle files
		const [pkg] = packagers.values();
		await require('../sourcemaps/sources')(
			distribution.maps,
			pkg,
			'ts',
			p.join(path, bundle.subpath.replace(/\./g, '/')),
			builder,
			language
		);
	};
	...
	const process = (packager, key) => {
		...
		if (!code.valid) {
			const [p] = key.split('//');
			const error = `  . Bundle "${bundle.specifier}" is not valid on platform "${p}"`;
		...
		const folders = bName.replace(/\./g, '/');
		const target = p.join(path, folders, `${bName}.${extension}`);
	...
	const { specifier } = bundle;
	if (exports.has(specifier)) {
		items.platforms = exports.get(specifier).platforms.concat(items.platforms);
		items.extensions = exports.get(specifier).extensions.concat(items.extensions);
	}
	exports.set(`${specifier}${language ?? ''}`, items);

	/**
	 * Memory heap fix
	 * clean ts compiler of each wrapper to free memory
	 */
	const cleanup = packager => {
		if (!packager.processors.has('ts')) return;
		const {
			packager: { compiler },
		} = packager.processors.get('ts');
		compiler.dispose();
	};
	packagers.forEach(cleanup);
};
