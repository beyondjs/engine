...
module.exports = async function (extname, specs) {
	...
	if (!code.valid) {
		...
		const error = `  . Bundle "${bundle.name}" on module "${name}" is not valid.`;
		builder.emit('error', error, { main: true, module: id });
		code.errors?.forEach(message => builder.emit('error', `    -> ${message}`, { module: id }));
		return;
	}
