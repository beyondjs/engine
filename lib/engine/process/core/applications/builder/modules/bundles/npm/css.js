...
module.exports = async function (specs) {
	...
	const buildCode = await require('../sourcemaps')(bundle, code, '.css', path, distribution.maps, 'css');
	// const target = p.join(path, module.subpath, `${exported}.css`);
	const target = p.join(path, bundle.subpath.replace(/\./g, '/'), `${exported}.css`); // multibundle
