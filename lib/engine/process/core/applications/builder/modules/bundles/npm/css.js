...
module.exports = async function (specs) {
    ...
    await code.ready;
    if (!code.valid) {
        builder.emit('error', `  . Bundle "${bundle.name}" on module "${module.name}" is not valid`, {module: module.id});
        return;
    }
    ...
    (distribution.maps, packager, 'sass', p.join(path, bundle.subpath), builder, language);
