...
module.exports = async function (bundle, path, builder) {
    const processDeclaration = async (declaration, tsConfig) => {
        ...
        try {
            ...
            console.error(`Declaration "${packager.bundle.container.name}" not process:`, exc.message);
        ...
        try {
            ...
            console.error(`tsconfig "${packager.bundle.container.name}" not process:`, exc.message);
            const error = `  . Could not generate "${packager.bundle.container.name}" tsconfig.`;
            builder.emit('error', error, {module: packager.bundle.container.id});
        }
    ...
    const {declaration, tsconfig} = await getDeclaration(bundle);
