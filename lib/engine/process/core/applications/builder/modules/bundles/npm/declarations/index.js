...
const getDeclaration = async (bundle, builder) => {
    ...
    if (!distribution) {
        builder.emit('error', `Distribution not found for bundle "${bundle.specifier}"`);
        return;
    }
    ...
    if (!processor) {
        builder.emit('error', `Processor "ts" not found for bundle "${bundle.specifier}"`);
        return;
    }
...
module.exports = async function (bundle, path, builder) {
    const processDeclaration = async (declaration, tsConfig) => {
        ...
        if (!valid || errors.length) {
            const error = `  . Module "${packager.bundle.container.name}" declaration not valid.`;
        ...
        try {
            ...
            const error = `  . Could not generate tsconfig file for module "${packager.bundle.container.name}".
    ...
    const d = await getDeclaration(bundle, builder);
    if (!d) return;

    const {declaration, tsconfig} = d;
    return processDeclaration(declaration, tsconfig);
}