...
module.exports = async function (extname, specs) {
    ...
    const id = builder.application.package !== module.container.package ?
               `packages/${bundle.vspecifier}` : subpath;

    const filename = id + (language ? `.${language}` : '') + extname;
