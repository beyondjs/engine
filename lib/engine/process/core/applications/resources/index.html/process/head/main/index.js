...
module.exports = async function (baseDir, application, distribution) {
    ...
    const required = await (async () => {
        ...
        distribution.local && distribution.development.tools && await push('@beyond-js/local/main');
    ...
    code += await (async () => {
        ...
        if (mode === 'sjs') return await require('./sjs')(baseDir, application, distribution, required);
        if (mode === 'amd') return await require('./amd')(baseDir, application, distribution, required);
        if (mode === 'esm') return await require('./esm')(baseDir, application, distribution, required);
        throw new Error(`Invalid distribution bundles mode "${mode}"`);
    })();

    // Add the code to fetch the page when ssr routing is on
    code += await require('./ssr')(application, distribution);
