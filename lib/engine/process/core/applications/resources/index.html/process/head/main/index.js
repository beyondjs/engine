...
module.exports = async function (baseDir, application, distribution, local) {
	...
	let code = '\n';
	code += '<script>\n';
	code += `globalThis.__app_package = ${__app_package};\n\n`;
	code += '// Set global baseDir and baseUrl variables\n';
	code += `const baseDir = globalThis.baseDir = '${baseDir}';\n`;
	code += 'const baseUrl = globalThis.baseUrl = (() => {\n';
	...
	const required = await (async () => {
		...
		if (application.engine !== 'legacy') {
			...
			await push('@beyond-js/styles/main');
		}
	...
	//     let c = minify(code);
	// c.error && console.log('error min', c.error)
