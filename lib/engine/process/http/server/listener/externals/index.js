...
module.exports = async function (url, application, distribution) {
    ...
    pathname = pathname.slice(0, pathname.length - ext.length);
    ...
    const {resource, pkg} = require('./parser')(pathname);
    ...
    const {code, map, dependencies, errors} = await require('uimport')(resource, mode, specs);
