...
module.exports = async function (bundle, packager, code, resource) {
    ...
    const done = ({errors, warnings, script, dependencies}) => {
        ...
        const content = JSON.stringify({script, dependencies, errors, warnings});
    ...
    const {hmr} = resource;
    const script = require('./sourcemap')(bundle, resource.is, code.code(hmr), code.map(hmr), '.js', 'inline');
    ...
    const {is} = resource;
    return done({script, dependencies, warnings, is});
}
