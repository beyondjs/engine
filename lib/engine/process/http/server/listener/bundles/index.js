...
module.exports = async function (url, application, distribution) {
    ...
    const {packager, code} = await (async () => {
        ...
        if (parsed.tsconfig) return {packager, code: packager.processors};
    ...
    await code.ready;
    if (!code.valid && !parsed.tsconfig) return new Resource({'500': `Errors found processing bundle code`});
    ...
    if (parsed.tsconfig) return await require('./tsconfig')(code);
