...
class ProcessorCompiler extends (require('./base')) {
    ...
    async _initialise() {
        const [bundleId, distributionId, processorName] = this.id.split('///');
        ...
        if (processorName < 1) return this._done(`Packager processor name "${this.id}" is invalid`);

        const packager = new model.Packager(this.id);
