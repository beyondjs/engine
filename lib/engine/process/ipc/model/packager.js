...
class Packager extends (require('./base')) {
    ...
    async _initialise() {
        ...
        await bundle.ready;
        if (bundle.error) return this._done(`Packager bundle not valid, ${module.error}`);
    ...
    toJSON() {
        const ids = [...this.item.processors.keys()].map(processor => `${this.id}///${processor}`)
        return {
            id: this.id,
            distribution: this.#distribution.id,
            processors: [...this.item.processors.keys()],
