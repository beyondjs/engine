module.exports = class {
    ...
    async _process({application, distribution, build, declarations}) {
        ...
        build && await require('./build')(appModel, distribution);

        if (!declarations) return;

        const distributionModel = new this.#model.Distribution(`${distribution}`);
        await distributionModel.ready;
        if (distributionModel.error) {
            console.error(`Application "${application}" not found`);
            return;
        }

        await require('./declarations')(appModel.item, distributionModel.toJSON());
    }
