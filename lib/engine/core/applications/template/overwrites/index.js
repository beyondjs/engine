...
module.exports = class extends DynamicProcessor {
	...
	constructor(application, config) {
		...
		super.setup(
			new Map([
				['config', { child: config }],
				['bundles', { child: application.bundlers }]
			])
		);
	}
	...
	_process() {
		...
		for (const [containerId, containerConfig] of Object.entries(value)) {
			...
			for (let [key, keyConfig] of Object.entries(containerConfig)) {
				if (key !== 'static' && !this.#application.bundlers.has(key)) {
					this.#warnings.push(
				...
				if (typeof keyConfig !== 'object') {
					this.#warnings.push(
						`Invalid "${key}" configuration on container ` + `"${containerId}", an object was expected`
					);
					continue;
				}

				if (key !== 'static') {
					const { bundler } = this.#application.bundlers.get(key);
					if (!bundler.template) {
						this.#warnings.push(
							`Bundle "${key}" specified in the container ` +
