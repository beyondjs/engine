...
module.exports = class extends DynamicProcessor {
	...
	get application() {
		...
		return container.is === 'application' ? container : container.application;
	}
	...
	constructor(container) {
		...
		const children = [['registry.bundlers', { child: this.application.bundlers }]];

		// If the container is an application.library, bundlers are actually supported only if they are legacy
	...
	_process() {
		...
		config?.forEach((config, name) => {
			...
			if (!bundle) {
				const { meta, settings } = this.application.bundlers.get(name);
		...
		super.clear(); // Do not use this.clear as it would destroy still used bundlers
