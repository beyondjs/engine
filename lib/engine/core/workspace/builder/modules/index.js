...
module.exports = class {
	...
	async process(builder, distribution, path, uglifier, processSpecs) {
		...
		builder.emit('message', 'Processing package modules');

		const { npm, platform } = distribution;
		const bridges = platform === 'backend' ? new (require('./bridges'))(path) : void 0;

		const { package: pkg } = builder;
		await pkg.modules.ready;
		for (const module of await pkg.modules.values()) {
			await module.ready;
			...
			if (!npm && !module.platforms.has(platform)) continue;

			//bundles are not generated when compiled with node distribution and the module is a package from another project
			if (pkg.package !== module.container.package && (npm || node.includes(platform))) {
				continue;
			}

			const id = pkg.package !== module.container.package ? module.specifier : module.subpath;
