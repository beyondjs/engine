 ...
 */
module.exports = async function (externals, pkg, paths) {
	for (const external of [...externals.keys()]) {
		...
		if (!(await fs.exists(p.join(pkg.path, 'node_modules', specifier, 'package.json')))) continue;

		const file = await fs.readFile(p.join(pkg.path, 'node_modules', specifier, 'package.json'), 'utf8');
		const json = JSON.parse(file);
		if (!json.static) continue;

		for (const file of Object.values(json.static)) {
			const source = p.join(pkg.path, 'node_modules', specifier, file);
