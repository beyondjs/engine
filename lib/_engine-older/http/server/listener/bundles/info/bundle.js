 ...
 */
module.exports = async function ({ bundle, code, extname }) {
	...
	content += `<p><strong>Specifier:</strong> ${bundle.specifier}</p>`;

	await code?.ready;
	const packager = code?.packager;
	packager && (content += `<p><strong>Packager hash:</strong> ${packager.hash.value}</p>`);

	content += `<h1>Bundle diagnostics report</h1>`;

	const errors = !bundle.valid ? bundle.errors : code?.errors || [];
	const { length } = errors;
	if (length === 0) {
	...
	for (const processor of processors.values()) {
		...
		if (diagnostics?.valid) {
			...
			const processList = (name, list) => {
				...
				content +=
					`<h3>Errors found on ${filtered.size} ` +
