	...
	async process(distribution) {
		...
		if (split[0] === 'packages') {
			...
			if (version && library.version !== this.#version) {
				this.#error = !library.valid
					? `Dependency "${library.pkg}" is invalid, check that it is installed or configured`
					: `Version "${this.#version}" differs from the registered version of the dependency "${
							library.version
					  }"`;
				return;
			}
		...
		const transversal = await (async () => {
			...
			await application.bundlers.ready;
			if (!application.bundlers.has(filename)) return;
			const specs = application.bundlers.get(filename);
		...
		const { bundle, language, tsconfig } = await (async () => {
			if (module.bundles.size === 1) {
				...
				const tsconfig = unknown[0] === 'tsconfig' ? unknown[0] : '';
				const language = unknown[0] !== 'tsconfig' ? unknown[0] : '';
				return { bundle, language, tsconfig };
			} else {
		...
		await bundle.ready;
		if (!bundle.multilanguage && language && !tsconfig) {
