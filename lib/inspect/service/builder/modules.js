...
module.exports = function (ipc) {
    ...
    this.create = async params => {
        try {
            ...
            if (!projectPath) return {status: 'ok', error: 'Package not exists'};

            const {Project, Module} = (require('./models'));
            const project = new Project(projectPath);
            await project.load();
            if (!project._exists) return {status: 'ok', error: 'Package not exists'};
            ...
            const pkgName = project.scope ? p.join(`@${project.scope}`, project.name) : project.name;
