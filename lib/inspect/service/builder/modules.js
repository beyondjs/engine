...
module.exports = function (ipc) {
	...
	this.create = async params => {
		try {
			...
			const tplPath = project.templatesPath();
			if (!(await fs.exists(tplPath))) {
				return { status: false, error: `${project.packagesTemplates()} package not installed.` };
			}
			...
			// const pkgName = project.name;
			// const path = p.join(project.modules.path, 'node_modules', pkgName);
			...
			// console.log('module create project:', project, project.id, project.path, project.name);
			// console.log('execute module declaration', 'modules/declarations/update', id);
