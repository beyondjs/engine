...
module.exports = async function (params, session) {
    ...
    const action = 'applications/modules/processors/dependencies/list';
...
const setRequest = requests => {
    ...
    for (const [requestId, request] of requests) {
        ...
        for (const filter of request.filter) {
            if (filter.field !== 'processor') continue;
            id = filter.value;
        }
        requestProcessed.push({requestId: requestId, processor: id});
...
const setResponse = (requests, ipcData) => {
    ...
    for (const request of requests) {
        const items = [];
        if (ipcData && ipcData.hasOwnProperty(request.processor)) {
            const dependencies = ipcData[request.processor];
            for (const source of dependencies) items.push({tu: Date.now(), data: source});
        }
