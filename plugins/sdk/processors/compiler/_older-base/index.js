...
module.exports = class extends DynamicProcessor() {
    ...
    _prepared(require) {
        ...
        if (analyzer && (!require(analyzer) || !analyzer.synchronized)) {
            return 'analyzer is not synchronized';
        }
    ...
    hydrate(cached) {
        ...
        let {hashes, files, extensions, diagnostics} = cached;

        this.#hashes.hydrate(hashes);

        const hydrate = (is, cached) => {
            const {processor} = this.#packager;
            const source = new this.CompiledSource(processor, is);
    ...
    toJSON() {
        ...
        const hashes = this.#hashes.toJSON();
        return {hashes, diagnostics, files: [...files], extensions: [...extensions]};
    }
}
