...
class Bundle extends require('./base') {
	...
	async _initialise() {
		...
		if (!SDK.bundles.has(name)) return this._done(`Bundle "${name}" is not registered ${this.id}`);
		...
		if (!module.bundles.has(name)) return this._done(`Module "${module.id}" does not have a bundle "${name}"`);
