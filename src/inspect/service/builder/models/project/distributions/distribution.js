...
module.exports = class Distribution extends require('../../file-manager') {
	...
	skeleton = [
		...
		},
		{
			...
			type: 'string',
			required: true,
			values: ['web', 'android', 'ios', 'deno', 'npm', 'node', 'backend', 'ssr']
		},
		{
			name: 'ports',
			type: 'object',
		...
		},
		{
			...
			type: 'object',
			required: false,
			properties: [{ name: 'compiler', type: 'string', values: ['tsc', 'transpiler'] }]
		},
		...
		{ name: 'maps', type: 'string', values: ['inline', 'externals'] },
		{
			name: 'bundles',
			type: 'object',
			properties: [{ name: 'mode', type: 'string', values: ['sjs', 'amd', 'cjs', 'esm'] }]
		},
