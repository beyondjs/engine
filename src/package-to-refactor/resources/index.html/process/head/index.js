...
module.exports = async function (application, distribution, local) {
	...
	platform === 'electron' && (baseDir = '');

	/**
	 * @TODO Include application and global styles from a module (instead of the template)
	 */
	// application.template.application.value &&
	// 	(head +=
	// 		`<link rel="stylesheet" type="text/css" id="beyond-application-styles" ` +
	// 		`href="${baseDir}styles.css" media="screen" />\n`);

	// application.template.global.value &&
	// 	(head +=
	// 		`<link rel="stylesheet" type="text/css" id="beyond-global-styles" ` +
	...
	await (async () => {
		...
		head += favicon
			? `<link rel="shortcut icon" type="${type}" href="${baseDir}${favicon}"/>\n`
