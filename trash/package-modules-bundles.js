...
module.exports = class extends DynamicProcessor {
	...
	_process() {
		...
		const changed = (() => {
			...
			let changed;
			changed = [...this].reduce(
				(prev, [key, value]) => prev || !updated.platforms.has(key) || updated.platforms.get(key) !== value,
				false
			);
			if (changed) return true;

			changed = [...this.#specifiers].reduce(
				(prev, specifier) => prev || !updated.specifiers.has(specifier),
				false
			);
			return changed;
		})();
